...
# Assuming the configuration key is MIN_PHONE_LENGTH based on earlier definition
...
def verify_data(df):
    ...
    # CRITICAL: We now select ALL necessary columns for the dashboard's enrichment logic
    # We remove the old, ambiguous 'email_raw' and 'phone' columns if they existed.
    
    # Note: We must explicitly keep all enrichment columns generated by the scraper 
    # (Lead Score, Attribute, Reason to Contact, City, Niche)
    
    # List of expected final columns from the scraper output:
    expected_cols = [
...
if __name__ == "__main__":
    ...
    if not df_clean.empty:
        ...
        # Write headers based on the expected columns for stability
        pd.DataFrame(columns=df_raw.columns.tolist()).to_csv(output_path, index=False)
        print("Warning: No verified leads were generated. Wrote empty headers based on raw columns.")
